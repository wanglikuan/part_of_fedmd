...
#---------------------------------------
        ...
        for idx in range(N_parties//4):  ####20个client，4种model，按数据分布分为5组，每组都有4种model###
...
###########################################################################################################################
        ...
        for idx in range(N_parties): 
            ...
            #                                     class_in_use=mod_private_classes,
            #                                     verbose=True)
            X_tmp, y_tmp = generate_test_private_data(X=X_test_private, y=y_test_private,
                                                    N_parties=1,
                                                    classes_in_use=mod_private_classes,
                                                    N_samples_per_class=(idx//4),
                                                    data_overlap=False)
            private_test_data_tmp[idx] = {"X{}".format(idx): X_tmp, "y{}".format(idx): y_tmp}
        del idx
...
###################新的选择private_test_data的函数################################
def generate_test_private_data(X, y, N_parties = 10, classes_in_use = range(11), 
    ...
    for cls in classes_in_use:
        ...
        idx = np.random.choice(idx, N_samples_per_class_list[N_samples_per_class][list_idx] * N_parties, 
                               replace = data_overlap)
        combined_idx = np.r_[combined_idx, idx]
        # for i in range(N_parties):           
        #     idx_tmp = idx[i * N_samples_per_class_list[N_samples_per_class][list_idx] : (i + 1)*N_samples_per_class_list[N_samples_per_class][list_idx]]  ##N_samples_per_class随n_i改变就可以改变priv_data取值
    ...
    X_incomplete, y_incomplete = X[combined_idx], y[combined_idx]          
    return X_incomplete, y_incomplete                
    # total_priv_data = {}
