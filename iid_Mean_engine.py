...
def get_models_logits(raw_logits, threshold, N_models):  #input:  
    ...
    for index in range(N_models):
        ...
        #
        #加一个排序的函数，返回deepcopy_cosdist[model_index][]为每个model升序或降序排列的cosdist，用于做min-max标准化
        deepcopy_cosdist[index].sort()
        #################################################################
    print("thresholdtopk:", deepcopy_cosdist)
    #judge threshold
    ...
    for index in range(N_models):
        for index2 in range(range_models):
            ...
            if cosdist[index][index2] > deepcopy_cosdist[index][0]:  #等同于 if true:
                print("index,index2,cosdist[index][index2],deepcopy_cosdist[index][0]:",index,index2,cosdist[index][index2],deepcopy_cosdist[index][0])
                #################################################################
                #tmp_logits.append(raw_logits[index2])
                #min-max标准化:
                tmp_logits.append(raw_logits[index2]*(cosdist[index][index2]-deepcopy_cosdist[index][0]))
                min_max_sum += (cosdist[index][index2]-deepcopy_cosdist[index][0])
                #直接标准化：
                # tmp_logits.append(raw_logits[index2]*cosdist[index][index2])
                # cos_sum += cosdist[index][index2]
                #################################################################
        ...
        #min-max标准化:
        print("min_max_sun of model {0} used:".format(index), min_max_sum)
        tmp_logits /= min_max_sum
        #直接标准化：
        # print("min_max_sun of model {0} used:".format(index), cos_sum)
        # tmp_logits /= cos_sum
        #################################################################
...
class FedMD():
    def __init__(self, parties, public_dataset,
        ...
        for idx, model in enumerate(parties):
            ...
            train_dataloader = DataLoader(data(total_private_data["X"], total_private_data["y"]), batch_size=batch_size,
                                          shuffle=is_shuffle,
                                          sampler=None, batch_sampler=None, num_workers=num_workers, drop_last=False)
            ###############改成根据model改变test_data##############
            test_dataloader = DataLoader(data(private_test_data["X"], private_test_data["y"]), batch_size=batch_size,
                                         sampler=None, batch_sampler=None, num_workers=num_workers, drop_last=False)
            
            # test_dataloader = DataLoader(data(private_test_data["X1"], private_test_data["y1"]), batch_size=batch_size,
            #                              sampler=None, batch_sampler=None, num_workers=num_workers, drop_last=False)
            ####################################################
    ...
    def collaborative_training(self):
        ...
        while True:
            ...
            for model in self.collaborative_parties:
                ...
                logits.append(predict(model, X_data, device))   #此时predict输出为一个model的5000个test数据的model输出，logits在此append所有model的输出
            ...
            for index, model in enumerate(self.collaborative_parties): #改了测试数据集的数据分布
                ##########################################改成根据model改变test_data#######################################
                # test_dataloader = DataLoader(data(private_test_data["X{}".format(idx)], private_test_data["y{}".format(idx)]), batch_size=batch_size,
                #                             sampler=None, batch_sampler=None, num_workers=num_workers, drop_last=False)
                ...
                print("model {0} use labels test:".format(index), self.private_test_data["y"])
                # else:
                #     dataloader =  DataLoader(data(self.private_test_data["X2"], self.private_test_data["y2"]), batch_size=32,
                #                           shuffle=True,
                #                           sampler=None, batch_sampler=None, num_workers=self.num_workers, drop_last=False) 
                #     print("model {0} use private_data_2 test".format(index))
                #####################################################################################
            ...
            for index, model in enumerate(self.collaborative_parties):
                ...
                train_dataloader = DataLoader(data(alignment_data["X"], logits), batch_size=self.logits_matching_batchsize,  ########## SWITCH: logits<=>logits_models[index]
                                              shuffle=True,
                                              sampler=None, batch_sampler=None, num_workers=self.num_workers, drop_last=False)
