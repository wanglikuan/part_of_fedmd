...
#---------------------------------------
        ...
        for idx in range(N_parties):
            private_data_tmp[idx], total_private_data_tmp[idx] \
                = generate_bal_private_data(X_train_private, y_train_private,
                                            N_parties=1,
                                            classes_in_use=mod_private_classes[idx],
                                            N_samples_per_class=N_samples_per_class,
                                            data_overlap=False)
        del idx
        ##########合并list与dict#################
...
###########################################################################################################################
        ...
        for idx in range(N_parties): 
            X_tmp, y_tmp = generate_partial_data(X=X_test_private, y=y_test_private,    #主要是mod_private_classes：relabel the selected private data 
                                                class_in_use=mod_private_classes[idx],
                                                verbose=True)
...
def generate_bal_private_data(X, y, N_parties = 10, classes_in_use = range(11), 
    ...
    for cls in classes_in_use:
        ...
        for i in range(N_parties):           
            idx_tmp = idx[i * N_samples_per_class : (i + 1)*N_samples_per_class]  ##N_samples_per_class随n_i改变就可以改变priv_data取值
